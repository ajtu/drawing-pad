...
@objc
public protocol DrawingPadDelegate: class {
    func didStart()
    func didFinish()
...
@objc
@IBDesignable open class DrawingPad: UIView {
    ...
    weak open var delegate: DrawingPadDelegate?
    ...
    open func setDrawing(_image: UIImage) {
        if incrementalImage == nil {
            let rectPath = UIBezierPath(rect: self.bounds)
            UIColor.white.setFill()
            rectPath.fill()
